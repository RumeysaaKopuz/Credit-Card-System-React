...
function Card(props: CardProp) {
  ...
  const maskCardNumber = (cardNumber: any) => {
    ...
    return maskedNumber.split('').map((val, index) => {
      if (val === '#' && currentIndex < cardNumber.length) {
        const currentChar = cardNumber[currentIndex];
        currentIndex++;
        return currentChar;
      }
      return val;
  ...
  {maskCardNumber(cardNumber).map((val, index) => (
    <CSSTransition
      ...
      timeout={250 + index * 100} // Değişiklik burada
      key={index}
    >
